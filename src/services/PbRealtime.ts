...
export class PbRealtime {
    ...
    on<T extends PbModelBase>(
        ...
        const keyOptions = (p.params || p.headers) ? `?options=${
            encodeURIComponent(
                jsonStringify({ query: p.params, headers: p.headers })
            )
        }` : '';
    ...
    async update() {
        try {
            ...
            const state = (this.getIsConnected() ? 'ok' : 'ko') + Object.keys(this.subscriptions).join(',');
            if (state === this.state) {
                return;
            }
            this.state = state;
