...
export class PbColl<T extends PbModelBase> {
  ...
  constructor(public client: PbClient, name: string) {
    this.name = name;
  ...
  private reqOptions(method: ReqMethod, idOrUrl: string, o: PbOptions<T>): ReqOptions {
    const url = idOrUrl.indexOf('/') === -1 ? `collections/${this.name}/records${idOrUrl?`/${idOrUrl}`:''}` : idOrUrl;
  ...
  findId(where: PbWhere<T>, o: PbOptions<T> = {}): Promise<string | null> {
    ...
    return this.page(where, 1, 1, { select: ['id' as PbKeys<T>], ...o }).then((r) => r.items[0]?.id || null);
  }
  ...
  update(id: string, data: PbUpdate<T>, o: PbOptions<T> = {}): Promise<T | null> {
    ...
    return this.call('PATCH', id, { data, ...o })
      .catch(error => {
  ...
  delete(id: string, o: PbOptions<T> = {}): Promise<void> {
    ...
    if (!id) throw toError('no id');
    return this.call('DELETE', id, { ...o, req: { resType: 'text', ...o.req } })
      .catch(error => {
